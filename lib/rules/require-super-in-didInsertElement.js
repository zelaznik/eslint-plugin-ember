...
//----------------------------------------------
// General rule - Call _super in didInsertElement lifecycle hooks
//----------------------------------------------

module.exports = {
  meta: {
    docs: {
      description: 'Enforces super calls in didInsertElement hooks',
      category: 'Possible Errors',
      recommended: true,
      url: 'https://github.com/ember-cli/eslint-plugin-ember/tree/master/docs/rules/require-super-in-didInsertElement.md'
    },
  ...
  create(context) {
    ...
    return {
      CallExpression(node) {
        ...
        const didInsertElementProperty = ember.getModuleProperties(node).find(property => property.key.name === 'didInsertElement');

        if (didInsertElementProperty) {
          const didInsertElementPropertyBody = didInsertElementProperty.value.body.body;
          const nodes = findStmtNodes(didInsertElementPropertyBody);
          const hasSuper = checkForSuper(nodes);
          if (!hasSuper) { report(didInsertElementProperty); }
        }
      },
    };
  }
